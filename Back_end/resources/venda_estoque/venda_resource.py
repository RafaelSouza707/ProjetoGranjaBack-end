...
class VendaResource(Resource):
    ...
    def get(self):
        ...
        ValidarAcessoGranja.validar_acesso_granja(
            user_id,
            granja_id
        )
    ...
    def post(self):
        ...
        with session_scope():
            ...
            for item in itens:
                ...
                data_item, error = validate_schema(
                    item_venda_schema,
                    item
                )
                ...
                ItemVendaService.criar(data_item)
    ...
    def put(self, id):
        ...
        if not json:
            return {"message": "Dados não informados"}, 400

        # Guarda os itens separadamente porque eles
        # pertencem à tabela ItemVenda
        itens = json.get("itens", [])

        data, error = validate_schema(
            schema,
            json,
            partial=True
        )
        ...
        with session_scope():
            venda = VendaService.buscar_por_id(id)

            atualizado = VendaService.atualizar(
                venda,
                data,
                itens
            )
            ...
            # Busca novamente os itens depois da atualização
            itens_atualizados = ItemVendaService.listar(id)

            resultado["itens"] = itens_venda_schemas.dump(
                itens_atualizados
            )
